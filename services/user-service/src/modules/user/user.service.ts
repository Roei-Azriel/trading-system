...
import {EmailAlreadyTakenError ,UsernameAlreadyTakenError , IdNumberAlreadyTakenError , IdIsIncorrect , InvalidPasswordError} from './user-errors.js'
...
export class UserService { //Create new class for the user Service
  ...
  const res = await fetch(`http://localhost:30003/wallet/${created.id}/createWallet`, {
    method: "POST",
  });  
  if(!res){
    throw new Error("Wallet service connection failed");
  }
  const result = await res.json();
  if(!result){
    throw new Error("Wallet service cannot create wallet");
  }
...
export const userService = new UserService(prismaUserRepo);
