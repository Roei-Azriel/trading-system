...
import {creditSchema , type DebitDTO , type CreditDTO , balanceSchema} from './wallet.types.js'
...
export async function createWallet(req: Request, res: Response) {
  ...
  try {
    ...
    console.error(err);
    return res.status(500).json({ message: "create wallet failed" });
  }
}
...
export async function getUserWallet(req: Request , res: Response){
  ...
  try {
    const result = await walletService.getWalletId(userId);
    return res.status(200).json(result);
  }catch(e){
    return res.status(500).json({message:'fetch wallet id is failed'})
  }
}
